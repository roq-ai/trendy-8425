...
import { getSavedSearches } from 'apiSdk/saved-searches';
...
import { getTrendSearches } from 'apiSdk/trend-searches';
import { TrendSearchInterface } from 'interfaces/trend-search';

import { SearchListPage as ShowCaseList } from 'pages/searches';

function DashboardPage() {
  const { session } = useSession();

  const savedSearchStat = useSWR<PaginatedInterface<SavedSearchInterface>>(
    () => `/saved-searches`,
    () => getSavedSearches({ limit: 1, offset: 0 }),
  );
  ...
  const trendSearchStat = useSWR<PaginatedInterface<TrendSearchInterface>>(
    () => `/trend-searches`,
    () => getTrendSearches({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Saved Searches',
      result: savedSearchStat?.data?.totalCount,
      isLoading: savedSearchStat.isLoading,
    ...
    {
      title: 'Trend Searches',
      result: trendSearchStat?.data?.totalCount,
      isLoading: trendSearchStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`searches`} passHref legacyBehavior>
                  <Button
