...
import { getSavedSearchById, updateSavedSearchById } from 'apiSdk/saved-searches';
...
function SavedSearchEditPage() {
  ...
  const handleSubmit = async (values: SavedSearchInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSavedSearchById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<SavedSearchInterface>({
    initialValues: data,
